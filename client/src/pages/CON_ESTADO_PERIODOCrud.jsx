        ...
        <div style={{ background: 'white', padding: '30px', borderRadius: '12px', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }}>
            ...
            <form onSubmit={handleSubmit} style={{ marginBottom: '30px', padding: '20px', background: '#f8fafc', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                ...
                <div style={{ marginTop: '20px' }}>
                    ...
                    {editingId && (
                        <button type="button" onClick={() => { setEditingId(null); setFormData({ ESP_NOMBRE: '', ESP_DESCRIPCION: '' }); }} style={{ marginLeft: '10px', padding: '10px 20px', background: '#94a3b8', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: '500' }}>
                            Cancelar
                        </button>
                    )}
                </div>
                        ...
                        {data.map(item => (
                            <tr key={item.ESP_ESTADO_PERIODO} style={{ borderBottom: '1px solid #e2e8f0' }}>
                                ...
                                <td style={{ padding: '12px' }}>
                                    ...
                                    <button onClick={() => handleDelete(item.ESP_ESTADO_PERIODO)} style={{ padding: '6px 12px', background: '#ef4444', border: 'none', color: 'white', borderRadius: '4px', cursor: 'pointer', fontSize: '13px' }}>Eliminar</button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
